...
| DQ-04 | `bpm` | Range [`40`,`250`] | **FLAG** (Keep, but add 'Review' tag) |
| DQ-05 | `artist_count` | Must be Numeric and `≥1` | **TRANSFORM** (Set to `Null`) |
| DQ-06 | `released_year` <br> `released_month` <br> `released_day` | Range [`1900`,`2026`] <br> Range [`1`,`12`] <br> Range [`1`,`31`] | **TRANSFORM** (Set to `Null`) |
| DQ-07 | `key` | Must be in defined Set {`C`,`C#`,`D`,`D#`,`E`,`F`,`F#`,`G`,`G#`,`A`,`A#`,`B`} | **TRANSFORM** (Set to `Null`) |
| DQ-08 | `mode` | Must be in defined Set {`Major`,`Minor`} | **TRANSFORM** (Set to `Null`) |
